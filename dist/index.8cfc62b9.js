...
function addMealSearch(mealData) {
    ...
    meal.innerHTML = `
      ...
      <div class="meal">
        <div class="random-recipe">
          <h3 class="random">${mealData.strMeal}</h3>
  ...
  `;
    ...
    mealsEl.addEventListener('click', function(e) {
        if (e.target.closest('.heart-btn')) {
            btn = e.target.closest('.heart-btn');
            if (!btn) return;
        ...
        if (e.target.closest('.random-recipe')) {
            btn = e.target.closest('.random-recipe');
            showMealInfo(mealData);
        }
    });
};
...
function addFavMeal(mealData) {
  ...
  `;
    ...
    favContainer.addEventListener('click', function(e) {
        const isClicked = e.target.closest('.clear');
        if (!isClicked) return;
        removeMealLS(mealData.idMeal);
        fetchFavMeals();
    });
...
},{}]},["kMnXP","6rimH"], "6rimH", "parcelRequire041d")
